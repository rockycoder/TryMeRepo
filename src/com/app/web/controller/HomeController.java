...
public class HomeController {
	...
	/*
	 * @RequestMapping(method = RequestMethod.GET, value = "{path}") public
	 * String index(@PathVariable String path, ModelMap model) { String message
	 * = "Hello World, Spring 3.0!"; model.addAttribute("message", message);
	 * return "index"; }
	 */

	/*
	 * @RequestMapping(method = RequestMethod.GET, value = "{path}") public
	 * String formSub(@PathVariable String path, ModelMap model) { try {
	 * List<Categories> catLst = service.getCategories(); List<Products> prodLst
	 * = service.getAllProductsByCategory(1); List<Products>
	 * popularProds=service.getPopularProducts(); List<Products> latestProducts
	 * = service.getLatestProducts(); List<Reviews> revLst =
	 * service.getReviews(); model.addAttribute("Category_List", catLst);
	 * model.addAttribute("latestProducts", latestProducts);
	 * model.addAttribute("Reviews_list", revLst);
	 * model.addAttribute("Popular_Products", popularProds); return "store"; }
	 * catch (ApplicationException ae) { return "error"; } catch (Exception e) {
	 * return "fatal_error"; }
	 * 
	 * }
	 */
	...
	// Request mapping and function to get all the details of mobile devices
	...
	/*
	 * @RequestMapping(method = RequestMethod.GET, value = "{mobiles}") public
	 * String Mobiles(@PathVariable String path, ModelMap model) { try {
	 * List<Products> allMobiles = service.getAllProductsByCategory(1);
	 * //List<Products> bestMobiles = service.getLatestProducts();
	 * model.addAttribute("All_Mobiles", allMobiles); return "Mobiles";
	 * 
	 * } catch (ApplicationException ae) { return "error"; } catch (Exception e)
	 * { return "fatal_error"; }
	 * 
	...
	// Request mapping and function to get all the details of tablet devices
	// available in the database
	...
	public String Tablets(ModelMap model) {
		try {
			...
			model.addAttribute("All_Mobiles", allTablets); // All_Mobiles need
															// to be changed
			return "Tablets";
		} catch (ApplicationException ae) {
			return "error";
	...
	/*
	 * @RequestMapping(method = RequestMethod.GET, value = "/Cameras") public
	 * String Cameras(ModelMap model) { try { List<Products> allCameras =
	 * service.getAllProductsByCategory(1); List<Products> bestCameras =
	 * service.getLatestProducts(); String message = "Latest in Cameras";
	 * model.addAttribute("All_Mobiles", allCameras); //All_Mobiles need to be
	 * changed
	 * 
	 * 
	 * return "Cameras"; } catch (ApplicationException ae) { return "error"; }
	 * catch (Exception e) { return "fatal_error"; }
	 * 
	 * }
	 */
	...
	/*
	 * @RequestMapping(method = RequestMethod.GET, value = "{path}") public
	 * String Cameras(@PathVariable String path, ModelMap model) {
	 * List<Categories> catList = service.getCategories(); String message =
	 * "Latest in Cameras"; model.addAttribute("Category_List", catList); return
	 * "Cameras"; }
	 */

	// Request mapping and function to get all the details of Laptop devices
	// available in the database
	...
	public String Laptops(ModelMap model) {
		try {
			...
			model.addAttribute("All_Mobiles", allLaptops); // All_Mobiles need
															// to be changed
			return "Laptops";
		} catch (ApplicationException ae) {
			return "error";
	...
	// Request mapping and function to get all the details of Games available in
	// the database
	...
	public String Gaming(ModelMap model) {
		try {
			...
			model.addAttribute("All_Mobiles", allGaming); // All_Mobiles need to
															// be changed
			return "Gaming";
		} catch (ApplicationException ae) {
			return "error";
	...
	// Request mapping and function to get all the details of Appliances
	// available in the database
	...
	public String Appliances(ModelMap model) {
		try {
			...
			model.addAttribute("All_Mobiles", allAppliances); // All_Mobiles
																// need to be
																// changed
			return "Appliances";
		} catch (ApplicationException ae) {
